...
int main() {
	...
	//array_demo_1();
	// ##Q.1	There are inside array_demo_1 - Done

	//array_demo_2();
	// ##Q.2	In array_demo_2 - Done
	...
	// Push adds elements to the top of the stack, in this case we are adding 5 variables to the push.
	// The pop() method removes the last element from the stack
	// In this case we are removing elements from the bottom up until the stack is empty.
	// ##Q.5	A stack has no [] or at() method - why?
	// Because the Stack is a class, and we are using its functions calls to perform tasks, that is why we are using the invocation () brackets.
	// [] This is used for arrays primarily for indexing arrays and setting how large an array size is.

	//queue_demo();
	// ##Q.6	What is the difference between a stack.pop() and a queue.pop() ?
	// Stack Pop removes the last index while a Queue Pop removes the first index.

	//list_demo();
	// ##Q.7	Can we access a list value using an int index? Explain.
	// No we cannot due to it being a doubly linked list and this does not support indexing as its designed for efficient insertions and deletions rather than random access.
	// ##Q.8	Is there a reason to use a list instead of a vector?
	// A list is mainly used when we need to frequently insert and delete elements and is also useful for when a container changes its size frequently.

	vector_demo();
	// ##Q.9	Was max_size and size the same? (Can they be different?)
	// size is the size the given array while max_size is the maximum number of elements the container is able to hold.
	// ##Q.10	Which ParticleClass constructor was called?
	// The constructor overloaded with the arguments (int, int) as upon initialization, we use that constructor when we pass in the two int variables upon creation.
	// ##Q.11	Were the ParticleClass instances deleted? If so, how?
	// Yes they were deleted when after being pushed back in the array.
	// This is because the Particle Class that was created only existed in the push_back process, and since it is a class, it automatically deconstructed itself to free up memory but the values were stores in the array before its destruction.
	// ##Q.12	Was the vector instance deleted? If so, how do you know this?
	// Yes the Vector instance was deleted as it is a template class so created inside the demo function score, so the moment we hit the end of demo function as it was created inside the scope of the function, meaning when the end of the scope was reached, it automatically deconstructed itself.
	// ##Q.13	Your IDE might suggest to use emplace_back instead of push_back. What does this mean?
	// My IDE recommends this because of modern C++ and using emplace_back will have bettern performance and efficiency rather than push_back.
...
void array_demo_1() {
	...
	// The <> brackets indicate a Template Type, so in this case it requires a variable type and a size.
	// ##Q.1.2 Why don't we need to write std:array here? (Is this good?)
	// It is up to the developer to include std::array or not, it may help with readability but again its up to the developer.
	// ##Q.1.3 Explain what the int and 3 indicate in this case?
	// the "int" indicates the variable type and "3" indicates the size of the array.
	...
	for (_Array_iterator<int, 3>  itr2 = a1.begin(); itr2 < a1.end(); itr2++) {
		cout << *itr2 << " ";
	}
	cout << endl;
	// ##Q.1.4 In the code above, what is the type of itr2?
	// itr2 is of type std::array<int, 3> as the auto operator converts it to the type _Array_iterator. (This is generally considered bad practice because it can be confusing what type the itr actually is.
	// Normally developers would put the actual type there (Array_iterator<int, 3>) instead of using auto.
	...
	// V is of type int& of a reference of an int.
	// ##Q.1.6 In the code above, what does the & mean in (auto& v : a1)
	// the "&" symbol stands for reference. meaning v& is a direct reference to a1.
	...
	// Because we are looking at an index that is out of bounds, arrays start at 0 not 1, so us looking at slot [3] is us looking at index 4 in the array which doesnt exist.
	...
	//Empty just outputs 0
	
	// sort?
	// This is a sorting algorithm that takes in the first element and the last and sorts them.
	sort(a1.rbegin(), a1.rend());
	cout << "Reverse Sort() on a1, now ..." << endl;
	showIntArray(a1);
	// ##Q.1.8 How would you do a forward (not reverse) sort?
	// We use .being() and .end()
	// rbegin is a function that returns the reverse iterator which points to the last element. same with rend, returns the first element.
...
void array_demo_2() {
	...
	//a1 is created with a size of 5, all elements in a1 however are garbage values since they are not initialized.
	array<int, 5> a1;
	array<int, 4> a2 = { -4, 2, 7, -100 };

	//We output the size of the a1 array in hexadecimal.
	...
	//We create a copy of a1 in a4 using the auto variable, so a4 gains the array type and size from a1 being <int, 5>
	auto a4(a1); // this works too
	//Exact same as above, a4's size is outputted as hexadecimal.
